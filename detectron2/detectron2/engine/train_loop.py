    ...
    def run_step(self):
        ...
        if self.iter % self.cfg.LOG_ARGS.LOG_EVERY_N == 0:
            ...
            for i in range(LOG_NUM_IMG):                    
                ...
                orig_width = data[i]['width']
                img_id = data[i]['image_id']      
                image = data[i]['image']            # (3 resized_H resized_W) [0,255]
                gt_anns = data[i]['instances']
                pred_anns = model_out[i]['instances']


                # gt annotation
                gt_img_np = image.clone().detach().cpu().numpy()  # (C, H, W)
                gt_img_np = np.transpose(gt_img_np, (1, 2, 0))       # to (H, W, C)
                # Convert to uint8 if needed (assuming float image in 0-1)
                if gt_img_np.dtype != np.uint8:
                    gt_img_np = (gt_img_np * 255).clip(0, 255).astype(np.uint8)
                # img_box = img_np.copy()
                ...
                # pred annotation
                pred_img_np = image.clone().detach().cpu().numpy()  # (C, H, W)
                pred_img_np = np.transpose(pred_img_np, (1, 2, 0))       # to (H, W, C)
                # Convert to uint8 if needed (assuming float image in 0-1)
                if pred_img_np.dtype != np.uint8:
                    pred_img_np = (pred_img_np * 255).clip(0, 255).astype(np.uint8)
                # img_box = img_np.copy()
                ...
                # Log gt ann
                gt_img_poly_tensor = torch.from_numpy(gt_img_poly).permute(2, 0, 1).float()  # (C, H, W)
                gt_img_poly_tensor = gt_img_poly_tensor / 255.0  # Normalize to [0, 1]
                # storage.put_image(f"VIS_train_gt/gt_poly_{img_id}", gt_img_poly_tensor)
                storage.put_image(f"VIS_train_gt/gt_poly_{i}", gt_img_poly_tensor)
                
                # Log pred ann
                pred_img_poly_tensor = torch.from_numpy(pred_img_poly).permute(2, 0, 1).float()  # (C, H, W)
